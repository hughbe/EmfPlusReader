//
//  EmfPlusRotateWorldTransform.swift
//
//
...
/// [MS-EMFPLUS] 2.3.9.3 EmfPlusRotateWorldTransform Record
/// The EmfPlusRotateWorldTransform record performs a rotation on the current world space transform.
/// The current world space transform is multiplied by this matrix, and the result becomes the new current world space transform.
...
public struct EmfPlusRotateWorldTransform {
    ...
    public init(dataStream: inout DataStream) throws {
        ...
        /// Type (2 bytes): An unsigned integer that identifies this record type as EmfPlusRotateWorldTransform from the RecordType
        /// enumeration.
        /// The value MUST be 0x402E.
        self.type = try RecordType(dataStream: &dataStream)
        guard self.type == .rotateWorldTransform else {
        ...
        /// Angle (4 bytes): A floating-point value that specifies the angle of rotation in degrees. The operation is performed by
        /// constructing a new transform matrix from the following diagram.
        /// sin(Angle)  cos(Angle)   0
        /// cos(Angle) -sin(Angle)  0
        /// Figure 3: Rotation Transform Matrix
        self.angle = try dataStream.readFloat(endianess: .littleEndian)
